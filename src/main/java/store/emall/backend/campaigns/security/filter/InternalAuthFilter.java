...
public class InternalAuthFilter extends OncePerRequestFilter {
    ...
    private boolean isInternalPath(HttpServletRequest request) {
        ...
        // POST /api/subscriptions/trial — called by Accounts
        if ("POST".equals(method) && "/api/subscriptions/trial".equals(path)) return true;
        // GET /api/offers/product/{productId}/active-price — called by Catalog, OrderHub
        if ("GET".equals(method) && path.matches("/api/offers/product/\\d+/active-price")) return true;
        if ("POST".equals(method) && path.matches("/api/offers/products/active-discounts")) return true;
